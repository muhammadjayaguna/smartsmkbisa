...
import { Cpu, Search, Save, AlertCircle, ArrowLeft } from 'lucide-react';
...
export default function GrowthMindsetPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: asmData } = await supabase
        ...
        .select('*')
        .eq('rombel_id', activeRombelId)
        .eq('jenis_asesmen', 'Growth Mindset');
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      if (upsertData.length > 0) {
        ...
        toast({ title: '✅ Data Growth Mindset berhasil disimpan!' });
        fetchData();
      } else {
  ...
  const stats = { Growth: 0, Fixed: 0 };
  Object.values(assessments).forEach(a => {
    if (a.hasil === 'Growth Mindset') stats.Growth++;
    if (a.hasil === 'Fixed Mindset') stats.Fixed++;
  });
  const totalAssessed = stats.Growth + stats.Fixed;

  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      <Link href="/administrasi-guru/wali-mindset" className="inline-flex items-center text-sm text-slate-500 hover:text-blue-600 transition-colors">
        <ArrowLeft className="w-4 h-4 mr-1" /> Kembali ke Pemetaan Pola Pikir
      </Link>
      
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-blue-100">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center text-blue-600">
            <Cpu className="w-5 h-5" />
          </div>
          <div>
            <h2 className="font-bold text-lg text-slate-800">Growth vs Fixed Mindset {activeRombel ? `- ${activeRombel.nama_rombel}` : ''}</h2>
            <p className="text-xs text-slate-500">Pemetaan kecenderungan pola pikir siswa dalam menghadapi tantangan</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button onClick={handleSaveAll} disabled={saving || loadingWali || loading} className="bg-blue-600 hover:bg-blue-700 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-blue-50 border-blue-100">
          <CardContent className="p-4 text-center">
            <p className="text-xs font-bold text-blue-600 uppercase">Growth Mindset</p>
            <h3 className="text-2xl font-black text-blue-700 mt-1">{stats.Growth} <span className="text-sm font-normal text-blue-400">Siswa</span></h3>
          </CardContent>
        </Card>
        <Card className="bg-slate-50 border-slate-200">
          <CardContent className="p-4 text-center">
            <p className="text-xs font-bold text-slate-600 uppercase">Fixed Mindset</p>
            <h3 className="text-2xl font-black text-slate-700 mt-1">{stats.Fixed} <span className="text-sm font-normal text-slate-400">Siswa</span></h3>
          </CardContent>
        </Card>
      </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-500 bg-slate-50/50 uppercase border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-6 py-4 font-bold w-48">Mindset</th>
                        ...
                        <td className="px-6 py-4">
                          <select 
                            ...
                            className={`w-full p-2 border rounded-lg text-sm outline-none transition-colors ${
                              asm.hasil === 'Growth Mindset' ? 'bg-blue-50 border-blue-200 text-blue-700 font-bold' : 
                              asm.hasil === 'Fixed Mindset' ? 'bg-slate-100 border-slate-300 text-slate-700 font-bold' : 
