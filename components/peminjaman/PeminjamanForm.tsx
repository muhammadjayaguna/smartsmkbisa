 ...
 import { Package, Calendar, Clock, User, FileText, Loader2, GraduationCap, Users } from "lucide-react";
 ...
 interface PeminjamanFormProps {
   ...
   setFormData: (data: PeminjamanFormData) => void;
 ...
 const PeminjamanForm = ({ formData, setFormData, onSubmit, onCancel, isLoading, isEdit = false }: PeminjamanFormProps) => {
   return (
     <Card className="border-primary/20 shadow-xl bg-gradient-to-br from-card to-card/50 backdrop-blur">
       <CardHeader className="pb-4">
         ...
         <CardDescription>
           {isEdit ? "Ubah data peminjaman barang" : "Isi form berikut untuk mengajukan peminjaman barang"}
         </CardDescription>
       </CardHeader>
       <CardContent>
         <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
           ...
           {/* Nama Alat */}
           <div className="space-y-2">
             <Label htmlFor="nama_alat" className="flex items-center gap-2">
               <Package className="h-4 w-4 text-primary" />
               Nama Alat/Barang
             </Label>
             <Input
               id="nama_alat"
               placeholder="Contoh: Proyektor, Laptop, dll"
